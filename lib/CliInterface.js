...
CliInterface.prototype.handleLine = function(buffer) {
  ...
  // This response is received for each player, so store in memory
  params = self.handle(buffer, "player id")
  if (params) {
    params = params.split(" ")
    var macAddress = params[1]

    this.knownMacAddresses.push(macAddress)

    self.emit('playerLoaded', macAddress)
    return true
  }
...
CliInterface.prototype.handlePlayerCommands = function(mac, cmdString) {
  ...
  if (startsWith("connected", cmdString)) {
    var connectedState = cmdString.match(/^connected\s(.*?)$/)[1]
    if (parseInt(connectedState) === 1) {
      connectedState = true
    } else {
      this.emit('playerDisconnect', mac)
      return true
    }
    this.emit(mac + ' connectedStateChange', connectedState)
    return true
  }
  ...
  if (startsWith("pause 1", cmdString)) {
    this.emit(mac + ' paused')
    return true
  }

  if (startsWith("play ", cmdString)) {
    this.emit(mac + ' play')
    return true
  }

  if (startsWith("client reconnect", cmdString) ||
    startsWith("client new", cmdString)) {

    this.emit('playerConnected', mac)
    return true
  }

  if (startsWith("client disconnect", cmdString)) {
    this.emit('playerDisconnect', mac)
    return true
  }
